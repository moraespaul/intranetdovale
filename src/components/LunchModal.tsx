...
const LunchModal = ({ open, onOpenChange }: LunchModalProps) => {
  ...
  const { data: historico = [], isLoading: loadingHistorico } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/HistoricoPedidos?usuario=${encodeURIComponent(loggedUser.nome)}`);
  ...
  const confirmOrder = useMutation({
    mutationFn: async (isForce: boolean = false) => {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/SolicitarAlmoco`, {
        method: "POST",
